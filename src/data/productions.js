...
import bandvote from "../assets/bandvote.png";
import bandvoteDetail1 from "../assets/bandvote-details1.png";
// import bandvoteDetail2 from "../assets/bandvote-details2.png";
...
export const productionInfo = [
    ...
    },
    {
        ...
        type: "design/code",
        category: "practice",
        description: "大学の軽音部で使うWeb投票管理サイト",
        technologies: ["Next.js", "TypeScript", "Firebase"],
        role: ["フロントエンド", "バックエンド"],
        url: "https://band-vote-iota.vercel.app/",
        github: "https://github.com/UTakuto/band-vote",
        awards: [],
        reflection: {
            strongPoints: [
                {
                    text: "投票の公平性を担保するため、各投票者の採点傾向を補正する独自のロジックを実装しました。具体的には、自分が出演したバンドを採点できないように制御しつつ、個人ごとに異なる補正係数を算出し、全体の点数に反映させることで、評価のバランスを整える仕組みを構築しました。また、出演バンドと採点対象のバンドに矛盾がある場合や、1〜10点以外の不正な点数が入力された際にはリアルタイムでエラー表示されるようUX設計にも工夫を加えました。さらに、投票データを集計・管理できる専用の管理者画面も実装し、集計結果を非公開で保持しながら、発表スタイルにも柔軟に対応できる仕様にしました。",
                    img: bandvoteDetail1,
                },
            ],
            challenges: [
                "現在は、ロジックのエラーが出ているのでそちらを6月中に修正していきたいと考えております。また今後は、出演バンド情報をCSVなどで一括登録できる仕組みの導入を検討しています。",
            ],
            learnings: [
                "採点ロジックの設計では、「公平な評価」という一見シンプルな目標の裏に、数学的な検証や例外処理の多さが潜んでいることを実感しました。また、他人による操作を通して初めて浮き彫りになるエッジケースも多く、ユーザーテストの重要性を改めて学びました。Firebaseを用いたバックエンドの構築では、便利さの一方でセキュリティルールの設計を慎重に行う必要があると痛感し、認証・権限設計の基礎力も養うことができました。加えて、使う人の目線に立ち、迷いなく操作できる導線設計やエラーメッセージの工夫の大切さを強く感じました。",
            ],
        },
    },
    // {
